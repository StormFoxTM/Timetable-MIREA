...
// Функция удаления каталога "temp" и его содержимого, а также создания каталога "last"
...
// Функция загрузки файлов в базу данных
func loader() error {
...
// функция для нахождения суммы md5
...
// функция которая считывает данные из excel и отправляет их в СУБД
...
// функция, которая считывает строку из ячейки excel
func cell_name(number int) string {
	name, _ := excelize.ColumnNumberToName(number)
	return name
}

// функция которая перемещает файл из папки temp в папку last
